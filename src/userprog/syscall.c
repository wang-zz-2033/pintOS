...
void Iwrite (struct intr_frame *f);//no f?    FIN
 ...
 * 
 * int wait (pid_t pid)    等待子进程pid并检索子进程的退出状态  "HARD TO FINISH"
 ...
 * 
 * bool remove (const char *file) 删除名为file的文件
 * 成功返回true，失败返回false。
 ...
 * 
 * int read (int fd, void *buffer, unsigned size) 从fd打开的文件中读取size字节到 buffer。
 * 返回实际读取的字节数（文件末尾为0）
